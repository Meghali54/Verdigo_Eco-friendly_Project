...
export function calculateHomeEmissions(data) {
  ...
  const gasEmissions = data.heatingType === 'gas' 
    ? data.gasConsumption * 12 * EMISSION_FACTORS.gas 
    : 0;
  
  // Oil heating emissions (assuming 100L per month if oil heating)
  const oilEmissions = data.heatingType === 'oil' 
    ? 100 * 12 * EMISSION_FACTORS.oil 
    : 0;
...
export function calculateTransportEmissions(data) {
  ...
  const carEmissions = data.carKmPerWeek * 52 * EMISSION_FACTORS.transport[data.carFuelType];
  
  // Public transport emissions
  const busEmissions = data.busKmPerWeek * 52 * EMISSION_FACTORS.transport.bus;
  const trainEmissions = data.trainKmPerWeek * 52 * EMISSION_FACTORS.transport.train;
  
  // Flight emissions (assuming average distances)
  const shortFlightEmissions = data.shortHaulFlights * 500 * EMISSION_FACTORS.transport.flight.shortHaul;
  const longFlightEmissions = data.longHaulFlights * 3000 * EMISSION_FACTORS.transport.flight.longHaul;
  
  return (carEmissions + busEmissions + trainEmissions + shortFlightEmissions + longFlightEmissions) / 1000;
}

export function calculateFoodEmissions(data) {
  // Calculate base emissions from servings
  const meatEmissions = data.meatServingsPerWeek * 52 * EMISSION_FACTORS.food.meat;
  const dairyEmissions = data.dairyServingsPerWeek * 52 * EMISSION_FACTORS.food.dairy;
  const grainsEmissions = data.grainsServingsPerWeek * 52 * EMISSION_FACTORS.food.grains;
  const fruitsEmissions = data.fruitsServingsPerWeek * 52 * EMISSION_FACTORS.food.fruits;
  const vegetablesEmissions = data.vegetablesServingsPerWeek * 52 * EMISSION_FACTORS.food.vegetables;
  
  const baseEmissions = meatEmissions + dairyEmissions + grainsEmissions + fruitsEmissions + vegetablesEmissions;
...
export function calculateWasteEmissions(data) {
  ...
  const recyclingItems = [data.recyclesPaper, data.recyclesPlastic, data.recyclesGlass, data.recyclesMetal];
  const recyclingRate = recyclingItems.filter(Boolean).length / recyclingItems.length;
