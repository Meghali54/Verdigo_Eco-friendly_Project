      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 text-lg border-2 border-border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-muted focus:bg-card"
            disabled={isLoading}
          />
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {quickCities.map((city) => (
            <button
              ...
              disabled={isLoading}
              className="bg-card hover:bg-accent border-2 border-border hover:border-primary text-foreground hover:text-primary font-medium py-3 px-4 rounded-lg transition-all duration-200 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {city}
            </button>
          ))}
